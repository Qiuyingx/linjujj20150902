...
public class KWSeeker {
	...
	private static volatile KWSeeker kSeeker = new KWSeeker();
	...
	private KWSeeker() {
		...
		//String filePath = GameUtils.class.getClassLoader().getResource("/sensitive.txt").getPath().replaceAll("%20", " ");
		File file = new File("resources/sensitive.txt");
		FileInputStream inputStream = null;
		try {
			inputStream = new FileInputStream(file);
		} catch (FileNotFoundException e) {
			LogUtil.logException("KWSeeker", e);
			System.err.println("敏感词词库文件没有发现！");
		}
		Scanner scanner = new Scanner(inputStream, "UTF-8");
		while (scanner.hasNextLine()) {
			String keyword = scanner.nextLine();
			if (StringUtils.isNotEmpty(keyword)) {
				sensitiveWords.add(new Keyword(keyword));
			}
		}
		scanner.close();
		initWords();
	 ...
	 * @return
	 */
	public static KWSeeker getInstance() {
		if (kSeeker == null) {
			kSeeker = new KWSeeker();
		}
		return kSeeker;
	}
	...
	/**
	 * 返回将传入的字符串使用指定的高亮方式将关键词标记出来的结果。如果没有指定formatter或text，则直接返回text。 这里使用指定的关键词词库进行处理。
	 * 
	 * @param text
	 *            传入需要处理的字符串。
	 * @param formatter
	 ...
	 */
	public Set<String> findWords(String text, AbstractFragment fragment) {
		text = EmojiUtil.filterEmoji(text);
